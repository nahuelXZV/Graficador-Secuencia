...
class CsharpController extends Controller
{
    ...
    public function generar(Request $request, $id)
    {
        ...
        $message = 'Necesito que me generes el codigo en c# para el siguiente diagrama de secuencia, toma en cuenta todo lo que esta en el texto, solo dame el codigo ningun comentario: ' . $diagrama->markdown;
        $response = $this->openIAController->sendMenssage($message);
        $code = $response['choices'][0]['message']['content'];
        $nameFile = $diagrama->nombre . '.cs';
